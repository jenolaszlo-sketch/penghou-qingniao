...
public sealed class SupervisionInterventionTests
{
    ...
    private static DelegationProgress CreateWaitingProgress(long revision = 2) => new(new DelegationId(Id(1)), DelegationState.WaitingForSupervisor, revision, [], [], 0, 0, DateTimeOffset.Parse("2026-01-01T00:00:00Z"), new SupervisorCheckpointDescriptor(new SupervisorCheckpointId(Id(10)), new SupervisionSessionReference("session-1"), new DelegationId(Id(1)), new DelegationAdmissionFence("plan", "implement", "1"), new WorkflowRunExecutionReference("zhinu", "run-1", "epoch-1"), new StructuralNodeReference("implement"), new NodeGenerationId(Id(11)), revision, true));
